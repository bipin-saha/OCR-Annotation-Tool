...
def update_previous_annotations(current_file_name):
    ...
    if current_index > 0:
        ...
        prev_annotation_label = tk.Label(previous_annotations_frame_inner, text="No previous annotations", bg="lightgrey")
        prev_annotation_label.pack()
...
def on_select(event):
    ...
    if item:
        ...
        selected_image_path = os.path.join(image_folder_path, selected_file_name)
...
def refresh_treeview():
    ...
    if os.path.exists(csv_file_path):
        ...
        if 'filename' not in df.columns or 'words' not in df.columns:
            messagebox.showerror("Error", "CSV file must contain 'filename' and 'words' columns.")
            return
...
def main():
    global root, tree, tree_selection, image_label, annotation_entry, previous_annotations_frame, check_mark_label, success_label
    root = tk.Tk()
    ...
    check_mark_label.pack_forget()  # Initially hide the label
